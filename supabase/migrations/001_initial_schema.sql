-- ============================================================
-- OptionPulse v2 — Supabase PostgreSQL Schema
...
CREATE TABLE option_chain_rows (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
CREATE TABLE market_regimes (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  instrument TEXT NOT NULL DEFAULT 'NIFTY',
  regime TEXT NOT NULL, -- TRENDING_BULLISH, TRENDING_BEARISH, SIDEWAYS, HIGH_VOLATILITY, LOW_VOLATILITY, CHOPPY
...
CREATE TABLE signals (
  ...
  signal_type TEXT NOT NULL, -- CALL_BUY, PUT_BUY, NO_TRADE, WATCH
  expiry DATE,
  strike INTEGER,
  ...
  status TEXT NOT NULL DEFAULT 'WATCH', -- WATCH, WAITING_FOR_ENTRY, ENTRY_TRIGGERED, POSITION_ACTIVE, TARGET1_HIT, TRAILING_SL, TARGET2_HIT, SL_HIT, EXIT, INVALIDATED, NO_TRADE
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
-- ============================================================
-- SIGNAL COMPONENTS (score breakdown)
-- ============================================================
CREATE TABLE signal_components (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  signal_id UUID NOT NULL REFERENCES signals(id) ON DELETE CASCADE,
  component TEXT NOT NULL, -- trend, price_action, vwap, momentum, volume, option_chain, oi, volatility, liquidity
...
-- ============================================================
-- SIGNAL EVENTS (state machine transitions)
-- ============================================================
CREATE TABLE signal_events (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  signal_id UUID NOT NULL REFERENCES signals(id) ON DELETE CASCADE,
  event_type TEXT NOT NULL, -- NEW_SIGNAL, ENTRY_TRIGGERED, TARGET1_HIT, TARGET2_HIT, SL_HIT, EXIT_TRIGGERED, INVALIDATED, STATUS_CHANGE
  from_status TEXT,
  to_status TEXT,
  nifty_price NUMERIC(10,2),
  option_price NUMERIC(10,2),
  reason TEXT,
  idempotency_key TEXT UNIQUE, -- signal_id + event_type for deduplication
...
CREATE TABLE paper_trades (
  ...
  pnl NUMERIC(10,2),
  status TEXT NOT NULL DEFAULT 'OPEN', -- OPEN, CLOSED_TARGET1, CLOSED_TARGET2, CLOSED_SL, CLOSED_MANUAL
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE backtest_trades (
  ...
  max_favorable NUMERIC(10,2),
  max_adverse NUMERIC(10,2)
);
...
CREATE TABLE system_logs (
  ...
  category TEXT, -- cron, signal, data, notification
...
CREATE INDEX idx_signals_strategy ON signals(strategy_version_id);
-- Note: DATE(created_at) index removed — not IMMUTABLE in PostgreSQL.
-- Use idx_signals_created_at with a WHERE created_at >= 'YYYY-MM-DDT00:00:00' filter instead.
...
CREATE INDEX idx_option_chain_snapshots_ts ON option_chain_snapshots(data_timestamp DESC);
CREATE INDEX idx_option_chain_rows_snapshot ON option_chain_rows(snapshot_id);
...
CREATE INDEX idx_market_quotes_ts ON market_quotes(data_timestamp DESC);
CREATE INDEX idx_market_quotes_instrument ON market_quotes(instrument);
CREATE INDEX idx_system_logs_created ON system_logs(created_at DESC);

-- ============================================================
-- ROW LEVEL SECURITY (enable, anon read-only for public data)
-- ============================================================
...
-- Public read policies (signals, market data visible to all)
...
CREATE POLICY "daily_summaries_read" ON daily_summaries FOR SELECT USING (true);
CREATE POLICY "market_quotes_read" ON market_quotes FOR SELECT USING (true);
CREATE POLICY "chain_snapshots_read" ON option_chain_snapshots FOR SELECT USING (true);
CREATE POLICY "chain_rows_read" ON option_chain_rows FOR SELECT USING (true);

-- Notification subscriptions: any anon can insert their own
CREATE POLICY "notif_sub_insert" ON notification_subscriptions FOR INSERT WITH CHECK (true);
CREATE POLICY "notif_sub_select" ON notification_subscriptions FOR SELECT USING (true);

-- Paper trades: any anon can insert/select
CREATE POLICY "paper_trades_all" ON paper_trades FOR ALL USING (true) WITH CHECK (true);

-- Service role bypass (all operations allowed server-side via service role key)

-- ============================================================
-- SEED: Default strategy version
-- ============================================================
INSERT INTO strategy_versions (name, version, parameters, weights, min_score, min_rr, is_active)
VALUES (
  'NIFTY-MULTICONFLUENCE',
  'V1.0',
  '{
    ...
    "min_liquidity_oi": 50000,
    "min_liquidity_volume": 1000,
    "max_bid_ask_spread_pct": 5.0,
    "avoid_choppy_adx_threshold": 20,
